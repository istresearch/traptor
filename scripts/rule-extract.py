...
def parse_ctd_rules(traptor_type,
    ...
    if traptor_type == 'follow':
        query = "select value from rules where type = 'username' and rule_set = '***REMOVED***'"
        ...
        query = "select value from rules where type = 'keyword' and rule_set = '***REMOVED***'"
        cursor.execute(query)
        values = [row[0] for row in cursor]
        rules = []
        for v in values:
